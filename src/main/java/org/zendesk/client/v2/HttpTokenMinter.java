 ...
 *
 * <p>Validates the token response by node type rather than coercing through {@code asText}/{@code
 * asLong} as the pagination handlers in {@code Zendesk} do. A malformed pagination field degrades
 * benignly, since pagination just stops; a malformed lifetime silently corrupts the refresh
 * schedule for every request that follows. Jackson coerces rather than rejects ({@code true} would
 * read as a one-second lifetime, a non-numeric string as zero), so the type checks make a bad grant
 * a loud {@link ZendeskOAuthException} instead of a plausible-looking expiry.
 *
